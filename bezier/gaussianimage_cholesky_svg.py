...
class GaussianImage_Cholesky(nn.Module):
    ...
    def sample_bezier_curves_unclose(self, control_points, num_samples, fine_samples=1000):
        ...
        # print(f"DEBUG: merged_points shape: {merged_control_points.shape}, samples_per_seg: {samples_per_segment}")
        
        if samples_per_segment <= 0:
             print("WARNING: samples_per_segment is 0 or negative!")
             return torch.empty((num_curves, num_samples, 2), device=device)

        sampled_points = self.sample_bezier_curves_uniform(merged_control_points,samples_per_segment)
        
        # print(f"DEBUG: sampled_points raw shape: {sampled_points.shape}")

        # Reshape back to (num_curves, num_samples, 2)
        # sampled_points shape is (num_curves * num_beziers, samples_per_segment, 2)
        # We want (num_curves, num_samples, 2). 
        # Note: num_beziers * samples_per_segment might be != num_samples due to int division.
        # Reshape to (num_curves, -1, 2) handles this.
        
        sampled_points = sampled_points.view(num_curves, -1, 2)
        # print(f"DEBUG: sampled_points final shape: {sampled_points.shape}")
    ...
    def densify(self, num, pos_init_method, gt_image, radii=0.02):
        ...
        if num <= 0:
            print("No curves to densify (num <= 0)")
            return

        # Collect centers, handling potential failures from pos_init_method
        centers_list = []
        for _ in range(num):
            try:
                ...
                # If pos_init_method fails (e.g., empty csize in sparse_coord_init),
                # fall back to random position
                h, w = gt_image.shape[2], gt_image.shape[3]
                centers_list.append([np.random.uniform(0, h), np.random.uniform(0, w)])

        if len(centers_list) == 0:
            print("No valid centers found for densification")
            return

        centers = torch.tensor(centers_list, dtype=torch.float32).to(self._control_points.device)
