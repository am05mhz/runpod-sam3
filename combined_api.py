...
# map component name -> script path (adjust if your folders change)
COMPONENT_PATHS = {
...
# Worker implementation (single worker, GPU-serialized)
async def worker_loop(worker_idx: int = 0):
    ...
    while True:
        ...
        job["status"] = "started"
        try:
            # Acquire GPU permit if job is GPU-heavy
            async with GPU_SEMAPHORE:
                ...
                try:
                    ...
                    # each component uses slightly different function names; job should specify 'callable'
                    ...
                    # call the function according to simple conventions:
                    # - supersvg.process_image_sam3(image_path, svg_path, png_path, **kwargs)
                    # - supersvg.process_image(image_path, svg_path, png_path, **kwargs)
                    # - bezier.run_bezier_splatting(job_id, image_path, args)
                    # - layeredsvg.run_vectorization(run_id) (we emulate by calling run_vectorization with a run_id-like path)
                    module_kwargs = job.get("kwargs", {})

                    # Call sync CPU/GPU-bound function in thread to avoid blocking event loop
                    result = await asyncio.to_thread(lambda: fn(*job.get("args", []), **module_kwargs))

                    # Persist result info
                    job["status"] = "completed"
                    job["result"] = result
                    # try to populate urls if output files exist
                    svg = job.get("svg_out")
                    png = job.get("png_out")
                    ...
                    # attempt to call module-specific cleanup if present
                    if hasattr(module, "unload_supersvg_model"):
                        try:
...
async def startup_workers():
    # single worker; GPU semaphore serializes heavy tasks
...
# -------------------------
# Routes (prefixed by original filenames)
# -------------------------

# 1) supersvg: enqueue a GPU-safe job that runs process_image_sam3 or process_image
...
async def supersvg_upload(
    ...
    JOBS[job_id] = {
        "module": "supersvg",
        "callable": "process_image_sam3",  # preferred entry
...
# 2) sam3: route that proxies the sam3 segment function (runs inline / on-demand)
@app.post("/sam3/segment")
async def sam3_segment(image: UploadFile = File(None), image_url: Optional[str] = Form(None), prompt: Optional[str] = Form(None), min_area: int = Form(0)):
    # import sam3 module and call its run_sam_inference wrapper
    ...
    # sam3 exposes run_sam_inference(...) coroutine; call via asyncio.to_thread where needed
    if image is None and image_url is None:
        raise HTTPException(status_code=400, detail="Provide image or image_url")
    if image_url:
        # call endpoint in module that handles URL (module has helper load_image_from_url)
        # fallback: call module.segment_endpoint if available
        if hasattr(sam3_mod, "load_image_from_url") and hasattr(sam3_mod, "run_sam_inference"):
            pil = await sam3_mod.load_image_from_url(image_url)  # coroutine in sam3/server.py
            img_np = sam3_mod.numpy.array(pil) if hasattr(sam3_mod, "numpy") else None
            # prefer using sam3_mod.run_sam_inference directly if available
        else:
            raise HTTPException(status_code=500, detail="sam3 helpers missing")
    else:
        contents = await image.read()
        # sam3 provides a segment endpoint; reuse its logic by calling run_sam_inference
        pil_img = None
        try:
        ...
        # call run_sam_inference (it's async in sam3/server.py -> wrap to_thread)
        out = await sam3_mod.run_sam_inference(img_np, prompt, None, None, None, min_area, 0.8, True)
        return JSONResponse(content={"segments": out})

# 3) bezier: enqueue bezier training job (CPU/GPU heavy)
...
async def bezier_upload(
    ...
    # Use bezier's run_bezier_splatting(job_id, image_path, args)
    job_id = uid
    JOBS[job_id] = {
        "module": "bezier",
        "callable": "run_bezier_splatting",
        "args": [job_id, inp, None],  # bezier expects args object for params; module will build if None
...
# 4) layeredsvg: enqueue v11 processing (uses run_vectorization in app11)
...
async def layeredsvg_upload(file: UploadFile = File(...), quality: str = Form("fast")):
    ...
    # For layeredsvg we call run_vectorization(run_id) defined in app11.py;
    # to adapt, we pass run_id as uid and point module to same run folder.
    job_id = uid
    JOBS[job_id] = {
        "module": "layeredsvg",
        "callable": "run_vectorization",
        "args": [job_id],  # app11.run_vectorization expects run_id str and uses its internal processing_status
        "kwargs": {},
        "status": "queued",
        "created_at": time.time(),
    }
    # ensure minimal processing_status in module will be filled when module is imported
    JOB_QUEUE.put_nowait(job_id)
    return {"job_id": job_id, "poll_url": f"/job/{job_id}/status"}

# Generic job status route
@app.get("/job/{job_id}/status")
async def job_status(job_id: str):
    job = JOBS.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    # present stable fields only
    return {
        "job_id": job_id,
